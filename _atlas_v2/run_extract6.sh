...
while [ $start -lt $TOTAL ]; do
  ...
  "$BL" --background "$BLEND" --python blend_extract6.py -- $start $BATCH > "blend_dump6/log_$start.txt" 2>&1
  # how many objs actually written in this batch's obj dir range?
  meta="blend_dump6/objects_$start.json"
  if grep -q "BATCHDONE $start" "blend_dump6/log_$start.txt"; then
    n=$(grep -oE '"gi": [0-9]+' "$meta" 2>/dev/null | tail -1 | grep -oE '[0-9]+')
    echo "  batch ok, last gi=$n"
    start=$((start+BATCH))
  else
    # crashed — resume just past the last written obj
    lastobj=$(ls blend_dump6/obj/*.obj 2>/dev/null | sed 's/.*\///;s/\.obj//' | sort -n | tail -1)
    crashline=$(grep "CRASHOBJ" "blend_dump6/log_$start.txt" | tail -1)
...
import json,glob,io
allm=[]
for f in sorted(glob.glob(r'blend_dump6\objects_*.json')):
...
json.dump(out, io.open(r'blend_dump6\objects.json','w',encoding='utf-8'), ensure_ascii=False)
